...
def efficient_frontier_and_min_var_data():
    target_returns, stds, weights = plot_efficient_frontier()
    min_var_weights, expected_return_of_min_var, std_of_min_var = calculate_minimum_variance_portfolio()
    st.session_state["target_returns"] = target_returns
    st.session_state["stds"] = stds
    st.session_state["weights"] = weights
    st.session_state["min_var_weights"] = min_var_weights
    st.session_state["expected_return_of_min_var"] = expected_return_of_min_var
    st.session_state["std_of_min_var"] = std_of_min_var
...
def plot_efficient_frontier():
    ...
    yearly_returns = st.session_state.get("yearly_returns")
    ...
    if short_bound != None or long_bound != None:
        target_returns, stds, weights = calculate_efficient_frontier(returns,yearly_returns,True,short_bound,long_bound)
    else:
        target_returns, stds, weights = calculate_efficient_frontier(returns,yearly_returns,False)
...
def calculate_minimum_variance_portfolio():
    ...
    if short_bound != None or long_bound != None:
        min_var_weights = calculate_min_var(returns, yearly_returns, True, short_bound, long_bound)
    else:
        min_var_weights, expected_return_of_min_var, std_of_min_var = calculate_min_var(returns, yearly_returns, False)
...
def rebalance_through_time():
    ...
    list_of_fee_costs
    total_return
    annual_return_of_portfolio
    """
    returns = st.session_state.get("returns")
    ef_returns = st.session_state.get("ef_returns")
    ...
    current_date = returns.index.min() + pd.DateOffset(years=ef_period)
    current_date = returns.index[returns.index > current_date][0]
    ...
    portfolio_value = 1
    fee_rate = 0.0075
    is_inf = isinstance(rebalance_distance, type(None))

    if not is_inf:     
        rebalance_distance = rebalance_distance/100

    yearly_returns_ef = cal_yearly_returns(ef_returns)

    if risk == 1:

        if bounded:
            target_returns_ef, stds_ef, weights_ef = calculate_efficient_frontier(ef_returns, yearly_returns_ef, True, short_bound, long_bound)
        else:
            target_returns_ef, stds_ef, weights_ef = calculate_efficient_frontier(ef_returns, yearly_returns_ef, False)
        if bounded:
            min_var_weights, _, _ = calculate_min_var(ef_returns,yearly_returns_ef,True,short_bound,long_bound)
        else:
            min_var_weights, _, _ = calculate_min_var(ef_returns,yearly_returns_ef,False)
        ...
        if risk == 'slope':
            _, _, weights_where_one = find_derivative(target_returns_ef,stds_ef,weights_ef)
